...
QRegion KisVisualRectangleSelectorShape::getMaskMap()
{
    ...
    if (m_type == KisVisualRectangleSelectorShape::border
        ...
        mask =
            mask.subtracted(QRegion(m_barWidth, m_barWidth, width() - (m_barWidth * 2), height() - (m_barWidth * 2)));
    }
    return mask;
}

QImage KisVisualRectangleSelectorShape::renderAlphaMaskImpl() const
{
...
{
    if (isHueControl() && colorSelector()->renderMode() == KisVisualColorSelector::StaticBackground) {
        return QImage();
    }
    return renderAlphaMaskImpl();
}
